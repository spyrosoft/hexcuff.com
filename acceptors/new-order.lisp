...
(defun invalid-input-exists (customer-email which-cuff slot-size which-ear quantity shopify-token)
  (let ((error-messages '()))
    ...
    (unless (and (check-type quantity integer) (< quantity 1))
      (push "Quantity must be an integer greater than or equal to one." error-messages))
    (unless (> quantity 100)
...
(defun send-order-confirmation-email (payment-gateway-response customer-email which-cuff slot-size which-ear quantity)
  (let ((shipping-data (jsown:val "source" payment-gateway-response)))
    (let ((customer-name (jsown:val "name" shipping-data))
          (address (jsown:val "address_line1" shipping-data))
          (city (jsown:val "address_city" shipping-data))
          (state (jsown:val "address_state" shipping-data))
          (zip (jsown:val "address_zip" shipping-data))
          (order-total (/ (parse-integer (gethash which-cuff *hex-cuffs-and-prices*)) 100))
          (order-id (jsown:val "id" shipping-data))
          (order-email-message (read-file-into-string "email-templates/order-received.txt")))
      (when (jsown:val "address_line2" shipping-data)
        (setq address (concatenate 'string address "
" (jsown:val "address_line2" shipping-data))))
